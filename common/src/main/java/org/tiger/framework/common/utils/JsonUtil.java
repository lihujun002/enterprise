...
public class JsonUtil
{
    ...
    static
    {
        ...
        gson = new Gson();
        mapper.getSerializationConfig().with(new SimpleDateFormat(DateUtil.DATE_TIME_FORMAT_YYYYMMDDHHMMSS));
        // 空值处理为空串
    ...
    public static <T> T fromStr(String json, Class<T> clz)
    {
        ...
        {
            logger.error("class:{} deSerial form Json {} has error:{}", clz, json, e.getMessage());
        }
        return deSerialObj;
    }
    ...
    public static <T> Collection<T> getCollectionFromStr(String json, Class<T> clz, Class<?> cls)
    {
        ...
        {
            logger.error("Collection of class:{} deSerial from json:{} has error:{}", clz, json, e.getMessage());
        }
        return deSerialList;
    }
